...
class UserView(Resource):
    ...
    def post(self):
        ...
        user = user_service.create(requests_json)
        return "", 201, {"location": f"/users/{user.id}"}


@user_ns.route("/password")
class UpdeteUserPasswordViews(Resource):
    def put(self):
        requests_json = request.json

        email = requests_json.get("email")
        old_password = requests_json.get("password_1")
        new_password = requests_json.get("password_2")
